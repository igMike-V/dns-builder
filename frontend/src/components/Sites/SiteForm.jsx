...
const SiteForm = ({
          ...
          if (s.id === site.id) {
            return {
              ...
              records: s.records.filter(
                (record) => record.siteRecord.id !== siteRecordId
              )
            }
          } else {
            return s
    ...
    setSites((prevSites) => {
      return prevSites.map((site) => {
        if (site.id === siteId) {
          return {
            ...site,
            records: [...site.records, siteRecord]
          }
        } else {
          return site
        }
      ...
      if (inputs[field].validator && inputs[field].validator === 'isDomain') {
        try {
          ...
          validator.setInvalid(
            setInputs,
            field,
            'error with domain input, check your entry and try again'
          )
        }
      }
      ...
      if (inputs[field].validator && inputs[field].validator === 'isIp') {
        try {
          ...
          validator.setInvalid(
            setInputs,
            field,
            'error with Ip Address input, input a valid IPv4 address and try again'
          )
        }
      }
    }
  ...
  return (
    <>
      <h1>
        {site ? `Edit Site: "${site.name} (${site.domain})"` : 'Add a site'}
      </h1>
                ...
                {showRecordForm && (
                  <div className="flex flex-row items-center justify-start gap-2">
                    ...
                    <button
                      ...
                      onClick={(e) =>
                        addRecord(e, site.id, recordInputs.record)
                      }
                    >
                      Add
                    </button>
                    <button
                      ...
                      onClick={(e) => handleCancelAddRecord(e)}
                    >
                      Cancel
                    </button>
                  </div>
